...
Dialog.prototype.addPanel = function(options) {
    try {
        ...
        var uuid = Utils.shortUUID();

        var title = options.get('title',    'Panel ' + uuid);
 ...
 * @returns {*}
 */
Dialog.prototype.addGroup = function(options) {
    try {
        ...
        var _options = new Base({
            ID      : 'group__' + Utils.shortUUID(),
            type    : FieldTypes.GROUP,
            margins : options.get('margins',  20),
            spacing : options.get('spacing',  4)
        });

        var _group = this.add(_options);
            _group.margins = _options.get('margins');
            _group.spacing = _options.get('spacing');

        this.logger.info(_group);

        module._groups.push(_group);
...
Dialog.prototype.add = function(options) {
    try {
        ...
        var _target = module.window // parent || module.window;
